...
@section('title')
AdminPanel | Edit city "{{ $city->title }}"
@endsection

@section('title', 'AdminPanel | Add city')

@section('main-admin-content')
    <h1>Редактировать город "{{ $city->title }}"</h1>
    ...
    <form action="{{ route('city.update', $city->id) }}" method="POST" class="admin-create-items-form" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="form-group">
            ...
            <select name="country_id" id="city_country_id" class="form-select">
                ...
                @foreach ($countries as $country)
                    <option value="{{ $country->id }}" @if ($country->id == $city->region->country->id) selected @endif >{{ $country->title }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="region_id" id="region_id" class="form-select">
                <option value="0" disabled>--- Выберете регион ---</option>
                <option value="{{ $city->region->id }}" selected>{{ $city->region->title }}</option>
            </select>
        </div>
        <div class="form-group">
            <label for="city_title">Город</label>
            <input type="text" name='title' placeholder="Введите название города" required class="form-control" id="city_title" value="{{ $city->title }}">
        </div>
        <div class="form-group">
            <button class="btn btn-success w-100" type="submit">Редактировать город</button>
        </div>
    </form>
