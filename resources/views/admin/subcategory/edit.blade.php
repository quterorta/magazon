...
@section('title')
AdminPanel | Add subcategory "{{ $subcategory->title }}"
@endsection
...
@section('main-admin-content')
    ...
    <form action="{{ route('subcategory.update', $subcategory->id) }}" method="POST" class="admin-create-items-form"  enctype="multipart/form-data">
        @csrf
        @method('PUT')
        <div class="form-group">
            ...
            <select name="chapter_id" id="category_chapter_id" class="form-select">
                ...
                @foreach ($chapters as $chapter)
                    <option value="{{ $chapter->id }}"@if ($subcategory->category->chapter->id == $chapter->id) selected @endif>{{ $chapter->title }}</option>
                @endforeach
            </select>
        </div>
        <div class="form-group">
            ...
            <select name="category_id" id="subcategory_category_id" class="form-select">
                ...
                <option value="{{ $subcategory->category->id }}">{{ $subcategory->category->title }}</option>
            </select>
        </div>
        <div class="form-group">
            <label for="subcategory_title">Название подкатегории</label>
            <input type="text" name='title' placeholder="Введите название подкатегории" required class="form-control" id="subcategory_title" value="{{ $subcategory->title }}">
        </div>
        <div class="form-group">
            <label for="subcategory_image">Изображение для подкатегории</label>
            <img src="{{ Storage::url($subcategory->image) }}" alt="" height: 100px style="display: block;">
            <input type="file" name='image' placeholder="Выберете изображение для подкатегории" class="form-control" id="subcategory_image">
        </div>

        <div class="form-group">
            <label for="subcategory_alias">Slug подкатегории (ЧПУ)</label>
            <input type="text" name='alias' placeholder="Slug подкатегории" required class="form-control" id="subcategory_alias" value="{{ $subcategory->alias }}">
        </div>
        <div class="form-group">
            <button class="btn btn-success w-100" type="submit">Редактировать подкатегорию</button>
        </div>
    </form>
